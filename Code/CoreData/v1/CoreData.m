...
#ifdef SMSYNCSERVER
#import <SMSyncServer/App-Swift.h>
#else
#import "App-Swift.h"
#endif
...
NSDictionary *_setupNames = nil;

- (NSURL *) sqliteURL {
    if (!_sqliteURL) {
        _sqliteURL = [[self applicationDocumentsDirectory] URLByAppendingPathComponent:_setupNames[COREDATA_SQLITE_FILE_NAME]];
    }
    return _sqliteURL;
}
...
- (NSURL *) sqliteBackupURL {
    if (!_sqliteBackupURL) {
        _sqliteBackupURL = [[self applicationDocumentsDirectory] URLByAppendingPathComponent:_setupNames[COREDATA_SQLITE_FILE_NAME]];
    }
...
- (void) setupWithMigrationOptions: (NSDictionary *) migrationOptions {
    _managedObjectModel = [self managedObjectModelForBundleResource:_setupNames[COREDATA_BUNDLE_MODEL_NAME]];
    if (!_managedObjectModel) {
        [self errorWithMessage: @"No managed object model"];
        return;
    }
    
    _persistentStoreCoordinator = [self persistentStoreCoordinatorWith:_managedObjectModel];
    if (!_persistentStoreCoordinator) {
        [self errorWithMessage:@"No persistent store coordinator"];
        return;
    }
    
    if (! [self addSqliteFileTo:_persistentStoreCoordinator withSqliteFileName:_setupNames[COREDATA_SQLITE_FILE_NAME] andOptions:migrationOptions]) {
        [self errorWithMessage: @"Could not add sqlite file to coordinator"];
        return;
    }
    
    _managedObjectContext = [self managedObjectContextWith:_persistentStoreCoordinator];
...
+ (instancetype) session {
    ...
    SPASLog(@"session: %@", s_sharedInstance);
...
- (NSManagedObjectModel *)managedObjectModelForBundleResource: (NSString *) modelResource
{
...
+ (NSManagedObjectContext *) defaultContextIfNil: (NSManagedObjectContext *) context;
{
    if (!context) {
        context = [[CoreData session] managedObjectContext];
    }
...
{
    context = [self defaultContextIfNil:context];
...
{
    context = [self defaultContextIfNil:context];
    return [self fetchObjectsInContext:context withEntityName:entityName error:error modifyingFetchRequestWith:nil];
}
...
{
    context = [self defaultContextIfNil:context];
    NSFetchRequest *fetchRequest = [self fetchRequestInContext:context withEntityName:entityName modifyingFetchRequestWith:fetchRequestModifier];
    
    NSArray *objs = nil;
    
    if (fetchRequest) {
        objs = [context executeFetchRequest:fetchRequest error:error];
    }
...
+ (NSUInteger) countOfObjectsInContext:(NSManagedObjectContext *) context withEntityName: (NSString *) entityName andError: (NSError **) error;
{
    *error = nil;
    ...
    if (*error || !context) {
        NSString *message = [NSString stringWithFormat:@"Error: %@; context= %@", *error, context];
        SPASLogFile(@"%@", message);
...
{
    [self removeObject:managedObject inContext:nil];
}
